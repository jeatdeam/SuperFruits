...
export const ProductsCards = () => {
    ...
    const refCarrito = useRef<HTMLButtonElement|null>(null);
            ...
            <main className={"w-full"}>
                ...
                <section className="w-[80%] border-[5px]  justify-center border-[purple] mx-auto flex flex-wrap content-start gap-[15px] relative" ref={refSectionProducts}>
                    {refProducts.current && refProducts.current?.map((el,key)=>(
                        <div className={"border-[2px]  border-[gray] rounded-[15px] w-[275px] h-[450px] px-[5px] py-[17.5px] flex flex-col items-center"} ref={(node)=>refPadre.current[key]=node} key={key} onClick={showInfoProduct}>
                            ...
                            <div className={"flex gap-[20px] justify-evenly w-full"}>
                                <button data-id={el.id} ref={refCarrito} onMouseDown={sendProduct} className={" bg-white  border-[2px] border-[gray] text-sm w-[80px] h-[40px] rounded-[8px] leading-[1.1]"}>agregar al carrito</button>
                                <Link to={"/productos/seccion-de-pagos"}>
